...
I've been building a [search engine](https://www.mtlarchives.com/) for 14,822 historical photos from the Montreal city archives. The collection goes from the 1870s to the 1990s. Aerial surveys, street photography, planning documents, index cards, all of it.

As part of that I generated [CLIP](https://openai.com/research/clip) embeddings (ViT-B/32, 512 dimensions) for every image and projected them down to 2D with UMAP. I figured I'd see clusters by subject. Churches in one corner, bridges in another, parks somewhere else.
...
Separation accuracy: 99-100%. Almost every 1940s photo sits apart from every 1950s+ photo. Not because of the content. Because of the border around the image.
...
2,781 index cards from the city's planning department cluster with a spread of 0.098 (vs 0.204 for aerial photos and 0.228 for oblique views). At deeper levels of hierarchical clustering the spread drops to 0.010. These cards look basically identical to CLIP.

Here's the part that got me: index cards from the 1950s and index cards from the 1970s are the same thing visually. The closest pair in the entire archive is two index cards 20 years apart, with an embedding distance of **0.00004**.

For context, two random photos from the same aerial flight path average 0.163. These cards are 4,000x closer than that.
...
Using k-means (k=8) on the full dataset:
...
I checked this with sequence analysis. Consecutive frames from the same flight average a distance of 0.059. Random frames from different flights average 0.368. That's a 6.2x difference.

```
Consecutive frames:  0.059  (baseline)
...
I found 1,840 pairs of photos from different decades that CLIP considers nearly identical:
...
Using nearest-centroid classification I found 20 high-confidence attributions for photos that didn't have credits. All went to Remillard since he has the biggest training set.

## The 6 outliers that aren't broken

Quality scoring across all 14,715 images: 99.96% scored above 0.3 on a 0-1 scale. Only 6 true outliers.

I expected damaged or faded photos. All 6 are from Flight 7P26 showing rural farmland on the edge of Montreal. Fields, ponds, agricultural land.
...
The silhouette score for semantic clustering is 0.18. Barely above random. Meanwhile, formatting-based clustering (documents vs raw photos) gets near-perfect separation.
...
**Outliers in clean datasets usually mean something.** Don't assume outliers are errors. The 6 photos that didn't fit weren't damaged. They were the only rural farmland in an urban archive. The model was right to set them apart.
